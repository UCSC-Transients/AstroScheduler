...
- [x] **Timeline Start Time Editing & Display timezone desync**: Save/load `currentTimezone` in `localStorage`, and update UI on change timezone without calling backend. Renamed labels in index.html to Display Timezone and Night Limits Timezone.
...
- [x] **Timeline Plot Tick & LST Alignment**: Pad timeline start/end to whole hours, draw vertical hourly grid lines, and render bottom UT/Local ticks and top LST ticks at whole hours.
